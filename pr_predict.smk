...
def get_encoding_information():
    ...
    for wl in [8, 11]:
        d = joblib.load(f"data/pds/models/best_model_0_wl_{wl}.joblib")
        ...
        for dataset, encoding in [name.split("-") for name in all]:
            ...
            ds = "pds/" + dataset.replace("pds", "pds_predict")
...
rule all:
    input:
         expand("data/pds/pds_predict_window_length_{window_length}/seqs.fasta",
                window_length=ENCODING_PROFILE["all"]["window_lengths"]),
         expand("data/pds/pds_predict_window_length_{window_length}/classes.txt",
                window_length=ENCODING_PROFILE["all"]["window_lengths"]),
         expand(f"data/pds/pds_predict_window_length_{{window_length}}_complete/seqs.fasta",
                window_length=ENCODING_PROFILE["complete"]["window_lengths"]),
         expand(f"data/pds/pds_predict_window_length_{{window_length}}_complete/classes.yaml",
                window_length=ENCODING_PROFILE["complete"]["window_lengths"]),
         expand(f"data/pds/pds_predict_window_length_{{window_length}}_complete/classes.txt",
                window_length=ENCODING_PROFILE["complete"]["window_lengths"]),
         ...
         # f"data/pds_predict/csv/predict/non_empty/all/"

rule utils_sliding_windows:
    input:
         series_in=f"data/pds/series_predict.yaml"
    output:
         fastas_out=expand(f"data/pds/pds_predict_window_length_{{window_length}}/seqs.fasta",
                           window_length=ENCODING_PROFILE["all"]["window_lengths"]),
         classes_out=expand(f"data/pds/pds_predict_window_length_{{window_length}}/classes.txt",
                            window_length=ENCODING_PROFILE["all"]["window_lengths"]),
    params:
...
rule utils_collect_encodings:
    ...
    output:
         sequence_based_encodings_out=\
             directory(f"data/pds_predict/csv/predict/sequence_based/"),
         structure_based_encodings_out=\
             directory(f"data/pds_predict/csv/predict/structure_based/"),
         csv_dir_out=\
             directory(f"data/pds_predict/csv/predict/non_empty/all/")
    params:
...
rule predict:
    input:
         "data/pds/models/best_model/best_model_0_wl_{window_length}.joblib",
         f"data/pds_predict/csv/predict/non_empty/all/"
    output:
         "data/temp/pds_predict/final_predictions_0_wl_{window_length}.yaml"
    run:
...
rule collect_models:
    input:
        expand("data/temp/pds_predict/final_predictions_0_wl_{window_length}.yaml",
               window_length=[8,11])
    output:
        "data/temp/pds_predict/final_predictions_0_complete.yaml"
    run:
        pass
