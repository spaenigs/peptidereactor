...
df_test = pd.read_csv("../data/bachem/csv/non_empty_test/bachem_window_length_11-waac_GEOR030106.csv", index_col=0)

with open("../data/temp/fba382c950e4/best_model_0_yprob.yaml") as f:
    probas = yaml.safe_load(f)

sequences, names = read_fasta("data/bachem_window_length_11/seqs.fasta")
seq_tuples = dict((name, tup) for name, tup in zip(names, sequences))

with open("../data/bachem/series.yaml") as f:
