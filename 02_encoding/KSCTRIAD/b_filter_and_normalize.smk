rule ksctriad_filter_datasets:
    ...
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,ksctriad}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_gap-{gapValue}.csv"
    group:
...
rule ksctriad_normalize:
    input:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_gap-{gapValue}.csv"
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,ksctriad}/csv/normalized/" + \
...
rule ksctriad_collect_normalized:
    input:
         collect_files
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,ksctriad}/csv/normalized/{dataset}_{part}_normalized-{normalized}.txt"
    run:
