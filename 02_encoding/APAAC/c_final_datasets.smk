...
rule generate_distance_matrix:
    ...
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/correlation/" + \
            "{dataset}_ifeature_apaacencoder_lambda-{lambdaValue}_normalized-{normalized}_vs_rest.csv"
    run:
        ...
        try:
            ...
            files = [f for f in
                     glob.glob(f"00_data/out/{wildcards.dataset}/{wildcards.dataset}_{wildcards.part}/encodings/apaac/csv/normalized/" + \
            ...
            pattern = f"00_data/out/{wildcards.dataset}/{wildcards.dataset}_{wildcards.part}/encodings/apaac/csv/normalized/" + \
...
def collect_files(wildcards):
    return expand( "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/correlation/" + \
                        "{dataset}_ifeature_apaacencoder_lambda-{lambdaValue}_normalized-{normalized}_vs_rest.csv",
...
rule collect_distance_matrix:
    input:
        collect_files
    output:
          "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/{dataset}_{part}_normalized-{normalized}_distance_matrix.csv"
    run:
...
rule run_clustering:
    input:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/{dataset}_{part}_normalized-{normalized}_distance_matrix.csv"
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/{dataset}_{part}_normalized-{normalized,yes|no}.csv"
    run:
        ...
        if not df.empty:
            ...
            df_for_cls["type"] = list(map(lambda x: re.match("apaacencoder_(.*)", x).group(1), res.index))
...
rule compute_geometric_median:
    input:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/{dataset}_{part}_normalized-{normalized}.csv"
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/geom_median/{dataset}_{part}_normalized-{normalized,yes|no}_{type}_vs_rest.csv"
    run:
...
rule collect_geometric_median:
    input:
        lambda wildcards: expand("00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/geom_median/" + \
                                 ...
                                 type=["lambda"])
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/{dataset}_{part}_normalized-{normalized}_geometric_median.csv"
    run:
        dfs = list(input)
        res = pd.DataFrame()
        for path in dfs:
...
rule plot_clustering:
    input:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/{dataset}_{part}_normalized-{normalized}_geometric_median.csv"
    output:
        "00_data/out/{dataset}/plots/{dataset}_{part}_apaac_normalized-{normalized}_tsne.svg"
    run:
        ...
        if not df_gm.empty:
            ...
            plt.title(f"{wildcards.dataset}{', normalized, ' if wildcards.normalized == 'yes' else ', ' }TSNE")
...
rule get_final_datasets:
    input:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/tsne/{dataset}_{part}_normalized-{normalized}_geometric_median.csv"
    output:
        temp("00_data/out/{dataset}/{dataset}_{part}/encodings/apaac/csv/final/geom_median/tsne/normalized-{normalized}/final_datasets.txt")
    run:
        ...
        if (not df_gm.empty) or (not df_gm.empty):
            ...
            with open(str(output), mode="a") as f:
                ...
                src = glob.glob(f"00_data/out/{wildcards.dataset}/{wildcards.dataset}_{wildcards.part}/encodings/apaac/csv/original/*{df_filtered.name.values[0]}.csv")[0]
                dst = f"00_data/out/{wildcards.dataset}/{wildcards.dataset}_{wildcards.part}/encodings/apaac/csv/final/geom_median/tsne/normalized-{wildcards.normalized}/{os.path.basename(src)}"
