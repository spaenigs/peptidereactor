rule eaac_filter_datasets:
    ...
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,eaac}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_window-{windowValue}.csv"
    group:
...
rule eaac_normalize:
    input:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_window-{windowValue}.csv"
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,eaac}/csv/normalized/" + \
...
rule eaac_collect_normalized:
    input:
         collect_files
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,eaac}/csv/normalized/{dataset}_{part}_normalized-{normalized}.txt"
    run:
