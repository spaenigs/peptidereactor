rule cksaagp_filter_datasets:
    ...
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,cksaagp}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_gap-{gapValue}.csv"
    group:
...
rule cksaagp_normalize:
    input:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_gap-{gapValue}.csv"
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,cksaagp}/csv/normalized/" + \
...
rule cksaagp_collect_normalized:
    input:
         collect_files
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,cksaagp}/csv/normalized/{dataset}_{part}_normalized-{normalized}.txt"
    run:
