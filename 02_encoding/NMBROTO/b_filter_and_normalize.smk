rule moran_filter_datasets:
    ...
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,moran}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_nlag-{nlagValue}.csv"
    group:
...
rule moran_normalize:
    input:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding}/csv/filtered/" + \
            "{dataset}_{part}_ifeature_{encoding}encoder_nlag-{nlagValue}.csv"
    output:
         "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,moran}/csv/normalized/" + \
...
rule moran_collect_normalized:
    input:
         collect_files
    output:
        "00_data/out/{dataset}/{dataset}_{part}/encodings/{encoding,moran}/csv/normalized/{dataset}_{part}_normalized-{normalized}.txt"
    run:
