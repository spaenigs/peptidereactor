...
# CUSTOMIZE
my $raptorx="/home/spaenigs/PycharmProjects/encoding_benchmark/snakemake/apps/RaptorX";
my $dbbase=$raptorx."/databases/NR_new/nr"; # will use PSI-BLAST dbs $dbbase."90" and $dbbase."70" 
                                   # It is recommended to use nre = nr + env. 
                                   # 90: filtered with CD-HIT to 90% maximum pairwise sequence identity
my $hh=".";
my $perl=$hh;
my $ncbidir=$raptorx."/util/BLAST/bin"; # directory with ncbi BLAST binaries
my $blastpgp=$ncbidir."/blastpgp -I T -s T"; # blastpgp executable
# our $blastpgp.=" -I T -s T"; # show gi's in defline; use Smith-Waterman
my $execdir=$raptorx."/util/PSIPRED/bin";  # directory with PSIPRED binaries 
my $datadir=$raptorx."/util/PSIPRED/data"; # directory with PSIPRED data 
...
my $maxiter=5;       # maximum number of psiblast iterations
