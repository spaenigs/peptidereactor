...
class Rule:
    ...
    def rule(self, fasta_in, fasta_msa_in, classes_in, path_to_config, pdb_dir, profile_dir, csv_dir, exclude=None, include=None):
        ...
        if "distance_distribution" in target_encodings:
            ...
            rule += encodings.distance_distribution.rule(fasta_in, classes_in, pdb_dir, distance_distribution_out)
        ...
        if "electrostatic_hull" in target_encodings:
            ...
            rule += encodings.electrostatic_hull.rule(fasta_in, classes_in, pdb_dir, electrostatic_hull_out)
            self.target_csvs += electrostatic_hull_out

        if "qsar" in target_encodings:
            qsar_out = f"{csv_dir}qsar.csv"
            rule += encodings.qsar.rule(fasta_in, classes_in, pdb_dir, qsar_out)
