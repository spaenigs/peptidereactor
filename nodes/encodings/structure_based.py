...
class Rule:
    ...
    def rule(self,
        ...
        if "asa" in target_encodings:
            ...
            rule += encodings.asa.rule(fasta_sec_in, classes_sec_in, profile_dir, asa_out, benchmark_dir)
            self.target_csvs += [asa_out]

        if "disorder" in target_encodings:
            disorder_out = f"{csv_dir}disorder.csv"
            rule += encodings.disorder.rule(fasta_sec_in, classes_sec_in, profile_dir, disorder_out, benchmark_dir)
            self.target_csvs += [disorder_out]

        if "disorderb" in target_encodings:
            disorderb_out = f"{csv_dir}disorderb.csv"
            rule += encodings.disorderb.rule(fasta_msa_sec_in, classes_sec_in, profile_dir, disorderb_out, benchmark_dir)
            self.target_csvs += [disorderb_out]

        if "disorderc" in target_encodings:
            disorderc_out = f"{csv_dir}disorderc.csv"
            rule += encodings.disorderc.rule(fasta_sec_in, classes_sec_in, profile_dir, disorderc_out, benchmark_dir)
            self.target_csvs += [disorderc_out]

        if "pssm" in target_encodings:
            pssm_out = f"{csv_dir}pssm.csv"
            rule += encodings.pssm.rule(fasta_sec_in, classes_sec_in, profile_dir, pssm_out, benchmark_dir)
            self.target_csvs += [pssm_out]

        if "sseb" in target_encodings:
            sseb_out = f"{csv_dir}sseb.csv"
            rule += encodings.sseb.rule(fasta_msa_sec_in, classes_sec_in, profile_dir, sseb_out, benchmark_dir)
            self.target_csvs += [sseb_out]

        if "ssec" in target_encodings:
            ssec_out = f"{csv_dir}ssec.csv"
            rule += encodings.ssec.rule(fasta_sec_in, classes_sec_in, profile_dir, ssec_out, benchmark_dir)
            self.target_csvs += [ssec_out]

        if "ta" in target_encodings:
            ta_out = f"{csv_dir}ta.csv"
            rule += encodings.ta.rule(fasta_sec_in, classes_sec_in, profile_dir, ta_out, benchmark_dir)
        ...
        if "delaunay" in target_encodings:
            ...
            rule += encodings.delaunay.rule(fasta_ter_in, classes_ter_in, pdb_dir, delaunay_out, benchmark_dir)
        ...
        if "distance_distribution" in target_encodings:
            ...
            rule += encodings.distance_distribution.rule(fasta_ter_in, classes_ter_in, pdb_dir,
                                                         distance_distribution_out, benchmark_dir)
        ...
        if "electrostatic_hull" in target_encodings:
            ...
            rule += encodings.electrostatic_hull.rule(fasta_ter_in, classes_ter_in, pdb_dir,
                                                      electrostatic_hull_out, benchmark_dir)
            self.target_csvs += electrostatic_hull_out

        if "qsar" in target_encodings:
            qsar_out = f"{csv_dir}qsar.csv"
            rule += encodings.qsar.rule(fasta_ter_in, classes_ter_in, pdb_dir, qsar_out, benchmark_dir)
