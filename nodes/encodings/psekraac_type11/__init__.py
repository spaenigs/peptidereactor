...
def _get_header(token):
    return f'''
rule encoding_psekraac_type11_{token}:'''
...
def _get_main(fasta_in, classes_in, psekraac_type11_out):
    return f'''
    input:
         fasta_in="{fasta_in}",
...
def rule(fasta_in, classes_in, psekraac_type11_out, benchmark_dir=None):
    ...
    Category: encodings \n
    Node: psekraac_type11

    :param fasta_in: The path to the fasta file.
    :param classes_in: The path to the classes file.
    :param psekraac_type11_out: A list of output file paths to store the encoded datasets.
