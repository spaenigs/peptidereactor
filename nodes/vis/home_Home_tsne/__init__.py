...
def _get_main(fastas_in, classes_in, benchmark_csvs_in, html_dir_out):
    return f'''
    input:
         fastas_in={fastas_in},
...
def rule(fastas_in, classes_in, benchmark_csvs_in, html_dir_out, benchmark_dir=None):
    token = secrets.token_hex(4)
    rule = _get_header(token)
    if benchmark_dir is not None:
        benchmark_out = f"{benchmark_dir}vis_home_Home_tsne_{token}.txt"
        rule += _get_benchmark(benchmark_out)
    rule += _get_main(fastas_in, classes_in, benchmark_csvs_in, html_dir_out)
    return rule
