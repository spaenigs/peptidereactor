...
def _get_header(token):
    return f'''
rule vis_mds_3_Clustering_{token}:'''
...
def _get_main(metric_dirs_in, html_dir_out):
    ...
    threads:
         1000
    params:
         snakefile="nodes/vis/mds_3_Clustering/Snakefile",
         configfile="nodes/vis/mds_3_Clustering/config.yaml"
    run:
...
def rule(metric_dirs_in, html_dir_out, benchmark_dir=None):
    """
    Creates a visualization for multiple datasets, i.e., a heatmap clustered by dataset and
    encodings.

    Category: vis \n
    Node: mds_3_Clustering
    ...
    if benchmark_dir is not None:
        benchmark_out = f"{benchmark_dir}vis_mds_3_Clustering_{token}.txt"
