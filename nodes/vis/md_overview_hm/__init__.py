...
def _get_header(token):
    return f'''
rule vis_md_overview_hm_{token}:'''
...
def _get_main(metric_dirs_in, html_dir_out):
    ...
    threads:
         1000
    params:
         snakefile="nodes/vis/md_overview_hm/Snakefile",
         configfile="nodes/vis/md_overview_hm/config.yaml"
    run:
...
def rule(metric_dirs_in, html_dir_out, benchmark_dir=None):
    ...
    if benchmark_dir is not None:
        benchmark_out = f"{benchmark_dir}vis_md_overview_hm_{token}.txt"
