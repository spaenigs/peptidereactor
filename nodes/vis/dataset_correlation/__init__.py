...
def _get_main(f1_csv_in, dataset_correlation_in, html_dir_out):
    return f'''
    input:
         f1_csv_in="{f1_csv_in}",
...
def rule(f1_csv_in, dataset_correlation_in, html_dir_out, benchmark_dir=None):
    token = secrets.token_hex(4)
    rule = _get_header(token)
    if benchmark_dir is not None:
        benchmark_out = f"{benchmark_dir}vis_dataset_correlation_{token}.txt"
        rule += _get_benchmark(benchmark_out)
    rule += _get_main(f1_csv_in, dataset_correlation_in, html_dir_out)
    return rule
