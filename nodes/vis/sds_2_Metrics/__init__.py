...
def _get_main(metrics_dir_in, html_dir_out):
    ...
    threads:
         1000
    params:
         snakefile="nodes/vis/sds_2_Metrics/Snakefile",
         configfile="nodes/vis/sds_2_Metrics/config.yaml"
    run:
...
def rule(metrics_dir_in, html_dir_out, benchmark_dir=None):
    ...
    different metrics for a dataset, grouped by encoding type and the
    results of the respective bootstraps.
    ...
    if benchmark_dir is not None:
        benchmark_out = f"{benchmark_dir}vis_sds_2_Metrics_{token}.txt"
