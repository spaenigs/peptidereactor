...
def get_hm(source, show_x_title=True, show_y_title=True):
    ...
    return alt.Chart(source).mark_rect().encode(
        ...
        color=alt.Color("Similarity_cat:N", scale=alt.Scale(domain=d, range=r), legend=alt.Legend(title="Similarity Range")),
...
if os.path.exists(p_div):
    ...
    df_div_clustered = get_clustering(df_div_1)
...
if os.path.exists(p_phi):
    ...
    df_phi_clustered = get_clustering(df_phi_1)
    df_phi_clustered.to_csv(p_phi)


hm1 = get_hm(df_div_clustered, show_x_title=False)
hm2 = get_hm(df_phi_clustered, show_x_title=False, show_y_title=False)
hm3 = get_hm(get_clustering(df_div))
hm4 = get_hm(get_clustering(df_phi), show_y_title=False)
...
(t1 & t2).configure_view(strokeWidth=0).save("hm.html")